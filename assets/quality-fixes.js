...
function colorScores(){document.querySelectorAll('.product-score').forEach(score=>{const n=parseFloat(score.querySelector('b')?.textContent);if(!Number.isFinite(n))return;score.dataset.scoreBand=n>=75?'high':n>=60?'good':n>=45?'mid':'low';const label=score.querySelector('span')?.textContent.trim()==='índice técnico'?'Índice técnico':'Índice calidad-precio';score.title=`${label}: ${Math.round(n)}/100. Cuanto más alto, mejor.`})}
